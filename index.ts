...
let lastDiagram: { title?: string; diagram: string; diagramType?: DiagramType } | null = null;
...
export const createDiagram = server.tool(
  ...
  async ({ title, diagram, diagramType }) => {
    ...
    return { content: [{ type: "text", text: `Created ${diagramType ?? "diagram"}${title ? `: ${title}` : ""}` }], structuredContent: data };
  }
);
...
export const editDiagram = server.tool(
  ...
  async ({ title, diagram, diagramType }) => {
    ...
    return { content: [{ type: "text", text: `Updated ${merged.diagramType ?? "diagram"}${merged.title ? `: ${merged.title}` : ""}` }], structuredContent: merged };
  }
);
